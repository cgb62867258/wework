...
class CorpAPI extends API
{
    ...
    {
//        log::info("发卡片消息了".json_encode($message));
    ...
    static private function _HttpPostXml($url, $args)
    {
        ...
        $corpApi = new CorpAPI();
        $corpApi->rspRawStr = HttpUtils::httpPost($url, $postData);
        return Utils::Xml2Array($corpApi->rspRawStr);
    }
    ...
    /**
     * @brief GetApprovalDetailByTempId : 使用sp_no获取审批申请详情
     *
     * @link https://work.weixin.qq.com/api/doc#10028/使用user_ticket获取成员详情
     *
     * @param sp_no : int
     *
     * @return : TempDetailByTempId
     */
    public function GetApprovalDetailBySpNo($sp_no)
    {
//        $config = include(__DIR__.'/../examples/config.php');
//        $agentId = $config['APP_ID'];
//        echo $agentId;die();

        Utils::checkNotEmptyStr($sp_no, "sp_no");
        $args = array("sp_no" => $sp_no);
        self::_HttpCall(self::APPROVAL_DETAIL, 'POST', $args);
        return $this->rspJson;
    }

    /**
     * @brief GetTempleteDetailBySpNo : 使用temp_no获取审批申请详情
     *
     * @link https://work.weixin.qq.com/api/doc#10028/使用user_ticket获取成员详情
     *
     * @param temp_no : string
     *
     * @return : TempDetailByTempId
     */
    public function GetTempleteDetailByTempNo($temp_no)
    {

        Utils::checkNotEmptyStr($temp_no, "template_id");
        $args = array("template_id" => $temp_no);
        self::_HttpCall(self::TEMPLATE_DETAIL, 'POST', $args);
        return $this->rspJson;
    }

    //提交审批单
    public function PostApproval($approvalData) {

        self::_HttpCall(self::POST_APPROVAL, 'POST', $approvalData);
        return $this->rspJson;
    }
    ...
    public function UploadTempMedia($file) {
        ...
        return $this->upImageToWork($url, $file);
    }
    //上传临时素材到企业微信
    public function upImageToWork($url, $file) {

        $curlPost = [
            'media' => new \CURLFile($file, 'image/jpg')
        ];
